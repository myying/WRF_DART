...
# plt.savefig("{:02d}.png".format(t), dpi=200)

###from model simulation
m = int(sys.argv[2])
plt.figure(figsize=(9, 5))
workdir = "/glade/scratch/mying/MPD/control/"
lat1 = wrf.getvar(workdir+"/rc/201906140000/wrfinput_d02", "XLAT")[0, :, :]
lon1 = wrf.getvar(workdir+"/rc/201906140000/wrfinput_d02", "XLONG")[0, :, :]
tmp = wrf.getvar(workdir+"/fc/20190614{:02d}15/wrfinput_d02_{:03d}".format(t-1,m+1), "RAINNC")[0, :, :]
tmp += wrf.getvar(workdir+"/fc/20190614{:02d}30/wrfinput_d02_{:03d}".format(t-1,m+1), "RAINNC")[0, :, :]
tmp += wrf.getvar(workdir+"/fc/20190614{:02d}45/wrfinput_d02_{:03d}".format(t-1,m+1), "RAINNC")[0, :, :]
tmp += wrf.getvar(workdir+"/fc/20190614{:02d}00/wrfinput_d02_{:03d}".format(t,m+1), "RAINNC")[0, :, :]
...
ax.text(-97.49, 36.61, '5', color='k', fontsize=10)
ax.set_title("1h precipitation (inch), 2019-06-14 {:02d}:00".format(t))
plt.savefig("{:02d}_{:03d}.png".format(t,m+1), dpi=200)
plt.close()
